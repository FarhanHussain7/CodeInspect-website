    ...
    <div className="fixed top-0 w-full z-50 bg-black/20 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-6 py-2 flex justify-between items-center">
        ...
        <Link to="/" onClick={handleScrollToTop} className="cursor-pointer flex items-center group">
          <svg xmlns="http://www.w3.org/2000/svg" width="250" height="60">
            <g transform="translate(0 10)">
              <circle cx="25" cy="25" r="20" fill="none" stroke="#2563EB" strokeWidth="3"/>
              <path stroke="#2563EB" strokeWidth="3" d="m40 40 15 15"/>
              <circle cx="25" cy="25" r="6" fill="#22c55e"/>
              <path stroke="#22c55e" strokeWidth="1.5" d="m19 20-4-5M31 20l4-5M19 30l-4 5M31 30l4 5"/>
            </g>
            <text x="70" y="30" fill="white" fontFamily="Arial, sans-serif" fontSize="20">CodeInspects</text>
            <text x="70" y="50" fill="white" fontFamily="Arial, sans-serif" fontSize="12">Automation Testing</text>
          </svg>
        </Link>

        {/* Desktop Nav (shows at sm and up) */}
        <div className="hidden sm:flex sm:items-center font-[Host_Grotesk] text-lg space-x-8 px-6 py-2 rounded-xl text-white whitespace-nowrap">
          <Link to="/" className={`relative group transition-all duration-300 ${getActiveClass('/')}`}> 
            <span onClick={handleScrollToTop} className="group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r from-blue-400 via-purple-400 to-pink-300 transition-all duration-300">Home</span> 
          </Link>
          <a href="#services" onClick={e => handleAnchorClick(e, '#services')} className={`relative group transition-all duration-300 ${getActiveClass('#services')}`}> 
          ...
          <a href="#about" onClick={e => handleAnchorClick(e, '#about')} className={`relative group transition-all duration-300 ${getActiveClass('#about')}`}> 
            <span className="group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r from-blue-400 via-purple-400 to-pink-300 transition-all duration-300">About Us</span> 
          </a>
          <a href="#events" onClick={e => handleAnchorClick(e, '#events')} className={`relative group transition-all duration-300 ${getActiveClass('#events')}`}> 
            <span className="group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r from-blue-400 via-purple-400 to-pink-300 transition-all duration-300">Events</span> 
          </a>
          <a href="#contact" className="relative px-8 py-2 font-bold text-lg text-white border-2 border-white rounded-xl group overflow-hidden transition-all duration-300 bg-zinc-300 hover:bg-gray-900" onClick={e => handleAnchorClick(e, '#contact')}>
            <span className="absolute inset-0 bg-gradient-to-r from-white/10 via-white/5 to-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out scale-110 group-hover:scale-100 blur-sm"></span>
            <span className="absolute inset-0 z-0 bg-white/5 group-hover:bg-white/10 transition-all duration-700 ease-in-out transform -translate-x-full group-hover:translate-x-0"></span>
            <span className="relative z-10 flex items-center justify-center gap-2 font-bold text-blue-500">Contact Now
